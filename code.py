...
def duplicate_and_unique_movies(dataset, index_):
    ...
    print('Number of duplicate Movies:', len(dup))
    print('\n')
    print('Examples of duplicate Movies:', dup[:15])
...
for movie in movies:
    name = movie[13]
    m_reviews = float(movie[12])
    
    if name in reviews_max and reviews_max[name] < m_reviews:
        reviews_max[name] = m_reviews
